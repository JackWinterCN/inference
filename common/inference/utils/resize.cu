...
#include "common/inference/utils/cuda_util.h"
#include "common/inference/utils/resize.h"
#include "common/inference/utils/util.h"
...
template <typename T>
__global__ void resize_linear_kernel_mean_std(
    const unsigned char *src, float *dst, int channel, int height, int width,
    int stepwidth, int dst_height, int dst_width, float fx, float fy, T mean_b,
    T mean_g, T mean_r, bool channel_axis, float std_b, float std_g,
    float std_r) {
  const int dst_x = blockDim.x * blockIdx.x + threadIdx.x;
  const int dst_y = blockDim.y * blockIdx.y + threadIdx.y;
  if (dst_x < dst_width && dst_y < dst_height) {
