...
int32_t RCNNProposalPlugin::enqueue(int32_t batchSize, const void *const *inputs, void *const *outputs,
  ...
  get_rois_nums(block_size, thread_size_, 0, stream, nthreads, rois,
                dev_batch_rois_nums);
  ...
  for (int batch_id = 0; batch_id < batchSize; ++batch_id) {
    ...
    get_max_score(block_size, thread_size_, 0, stream, nthreads,
                  decoded_bbox_pred + size_t(cur_ptr * (num_class_ + 1) * 4),
                  cls_score_softmax + size_t(cur_ptr * (num_class_ + 1)),
                  num_class_, threshold_objectness_, dev_thresholds, max_bbox,
                  max_score, max_all_probs, max_filtered_count);
