...
#include "common/inference/tensorrt/plugins/rpn_proposal_ssd_plugin.h"
...
//       temp_rpn_bbox_pred);
  reshape_boxes(block_size, thread_size_, 0, stream, nthreads, rpn_bbox_pred,
                height_, width_, num_anchor_per_point_, temp_rpn_bbox_pred);
      // Normalization
      float *dev_bbox_mean,
      *dev_bbox_std;
  BASE_GPU_CHECK(
...
//       dev_anchor_widths, anchors);
  generate_anchors(block_size, thread_size_, 0, stream, height_, width_,
                   heat_map_a_, num_anchor_per_point_, dev_anchor_heights,
                   dev_anchor_widths, anchors);
  // decode bbox
  float *proposals;
...
//       temp_scores);
  reshape_scores(block_size, thread_size_, 0, stream, nthreads,
                 rpn_cls_prob_reshape, height_, width_, num_anchor_per_point_,
                 temp_scores);
