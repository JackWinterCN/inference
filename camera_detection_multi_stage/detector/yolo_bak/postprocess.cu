...
#include "camera_detection_multi_stage/detector/yolo/object_maintainer.h"
...
int get_objects_gpu(
  ...
  auto &indices = const_cast<std::map<base::ObjectSubType, std::vector<int>> &>(
      indices_cns);
  auto &conf_scores =
      const_cast<std::map<base::ObjectSubType, std::vector<float>> &>(
          conf_scores_cns);
